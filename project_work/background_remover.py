...
def remove_background(img, top_bottom = False):
    ...
    for y in range(img.height):
        ...
        for x in range(img.width):
            if x == 0 or x == 1:
                pixels_new[img.width-(x+1), y] = (0, 0, 255, 255)
            elif pixels[img.width - x, y] == pixels[img.width-(x+1), y]:
                pixels_new[img.width - x, y] = (0, 0, 255, 255)
            else:
                pixels_new[img.width - x, y] = (0, 0, 255, 255)
    ...
    if top_bottom:
        for x in range(img.width):
            ...
            for y in range(img.height):
                if y == 0 or y == 1:
                    pixels_new[x, img.height - (y+1)] = (0, 0, 255, 255)
                elif pixels[x,img.height - y] == pixels[x, img.height - (y+1)]:
                    pixels_new[x, img.height - y] = (0, 0, 255, 255)
                else:
                    pixels_new[x, img.height - y] = (0, 0, 255, 255)
                    # pixels_check[img.width - x, y] = pixels[x, y]
                    break
...
if __name__ == '__main__':
    img = Image.open(r"C:\Users\User\Documents\GitHub\firstrepo\project_work\queen.png")
